...
Workflow executions are long-running (typically ~15 minutes). Follow this exact procedure:

1. **Launch as a background terminal** — Use `isBackground=true` (or equivalent). The workflow runs for ~15 minutes; it cannot block the chat.
2. **Do NOT check terminal output** — Do NOT call `get_terminal_output`, read output files, or attempt to verify the workflow started. The workflow takes several seconds to produce initial output, so early checks show nothing useful and waste time.
3. **End the conversation immediately** — Tell the user the workflow is running in the terminal and to watch the output there. End with something witty, creative, or funny. **Then stop. Do not continue the chat.**

```bash
# Execute (background terminal — workflows take ~15 minutes)
conductor -V run workflow.yaml --input key="value"
```

> **CRITICAL: After launching the workflow, your job is done.** Do NOT:
> - Call `get_terminal_output` to check if it started
> - Read any output files to see progress
> - Narrate what the workflow is doing
> - Explain the workflow stages or expected duration in detail
>
> Just tell the user to watch the terminal and end the conversation. Trust that the command started successfully.
...
# Override provider
conductor -V run workflow.yaml -p claude
